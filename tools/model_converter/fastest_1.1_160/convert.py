...
from tensorflow.keras.layers import (Conv2D, DepthwiseConv2D, Input, ZeroPadding2D, Add, Multiply, Lambda, Dropout,
                                     MaxPooling2D, AveragePooling2D, Concatenate, Activation, UpSampling2D)
...
parser.add_argument('--config_path', required=True, type=str, help='Path to Darknet cfg file.')
parser.add_argument('--weights_path', required=True, type=str, help='Path to Darknet weights file.')
parser.add_argument('--output_path', required=True, type=str, help='Path to output Keras model file.')
...
def _main(config_path, weights_path, output_path):
    ...
    if width and height and fixed_input_shape:
        input_layer = Input(shape=(height, width, nums_of_channel), name='image_input')
    else:
        input_layer = Input(shape=(None, None, nums_of_channel), name='image_input')
    prev_layer = input_layer
    all_layers = []

    count = 0
    out_index = []
    shortcut_activation_index = []
    for section in cfg_parser.sections():
        ...
        if section.startswith('convolutional'):
            ...
            if activation == 'linear':
                ...
                act_layer = ReLU(max_value=6)(prev_layer)
            ...
            # To support the shortcut with leaky ReLU
            # shortcut with leaky ReLU may make the layer index shift
            # ===============================================================
            for i in range(len(ids)):
                if ids[i] > 0:
                    shift = len([1 for index in shortcut_activation_index if ids[i] >= index])
                    ids[i] += shift
                else:
                    positive_index = ids[i] + len(all_layers) - 1
                    shift = len([1 for index in shortcut_activation_index if positive_index < index])
                    ids[i] -= shift
            # ===============================================================
            ...
            # ===============================================================
            if index > 0:
                shift = len([1 for shortcut_index in shortcut_activation_index if index >= shortcut_index])
                index += shift
            else:
                positive_index = index + len(all_layers) - 1
                shift = len([1 for index in shortcut_activation_index if positive_index < index])
                index -= + shift
            # ===============================================================

            if prev_layer.shape.as_list() == all_layers[index].shape.as_list():
                all_layers.append(Add()([all_layers[index], prev_layer]))
            else:
                # for support PRN(Partial Residual Networks)
                # Reference paper:
                # https://openaccess.thecvf.com/content_ICCVW_2019/papers/LPCV/Wang_Enriching_Variety_of_Layer-Wise_Learning_Information_by_Gradient_Combination_ICCVW_2019_paper.pdf
                #
                min_channel = min(all_layers[index].shape[3], prev_layer.shape[3])
                all_layers.append(Add()([all_layers[index][:, :, :, :min_channel], prev_layer]))
            prev_layer = all_layers[-1]
            ...
            if activation == 'leaky':
                ...
                # act_layer = ReLU(max_value=6)(prev_layer)
                all_layers.append(act_layer)
                shortcut_activation_index.append(len(all_layers) - 1)
            ...
            #
            # vela don't support nearneighbor
            #
            # method = tf.image.ResizeMethod.BILINEAR
            method = tf.image.ResizeMethod.NEAREST_NEIGHBOR
            all_layers.append(
                tf.image.resize(prev_layer, [prev_layer.shape[1] * stride, prev_layer.shape[2] * stride], method))
            prev_layer = all_layers[-1]
    ...
    weights_file.close()
    assert remaining_weights == 0, 'Warning: {} unused weights'.format(remaining_weights)
