...
def annotation_parse(annotation_lines, class_names):
    ...
    for line in annotation_lines:
        ...
        for box in boxes:
            ...
            # print(int(box.split(',')[-1]))
            if int(box.split(',')[-1])!=0:
                continue
...
def yolo_predict_tflite(interpreter, image, anchors, num_classes, conf_threshold, iou_threshold, elim_grid_sense, v5_decode):
    ...
    image_data = preprocess_image(image, model_image_size, norm = False, grayscale=True)
...
def main():
    ...
    parser.add_argument(
        '--json_name', type=str, required=True,
